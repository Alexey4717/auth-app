...
class TokenService {
    generateTokens(payload) {
        const accessToken = jsonwebtoken_1.default.sign(payload, process.env.JWT_SECRET_ACCESS, {
            expiresIn: "1h",
        });
    ...
    validateAccessToken(token) {
        try {
            const userData = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET_ACCESS);
            return userData;
        }
        catch (_a) {
            return null;
    ...
    validateRefreshToken(token) {
        try {
            const userData = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET_REFRESH);
            return userData;
        }
        catch (_a) {
            return null;
